...
OTRGlobals::OTRGlobals() {
    ...
    // TEST -----
    LUS::Scripting::onRegisterGlobals = [this](LUS::Mod* mod) {
        auto& env = mod->GetEnvironment();
        env["addMoney"] = [this](int16_t amount) { Rupees_ChangeBy(amount); };
        env["addHealth"] = [this](int16_t amount) { Health_ChangeBy(gPlayState, amount); };
        env["time"] = []() { return gSaveContext.sohStats.playTimer; };
    };
    // --------

    LUS::Scripting::Load(); // Change this to LoadMod and get the folder from .otr
    LUS::Scripting::Call("init");
    // ---------------------------------------
...
extern "C" int CustomMessage_RetrieveIfExists(PlayState* play) {
    ...
    if (gSaveContext.n64ddFlag) {
        ...
        } else if (play->sceneNum == SCENE_TREASURE_BOX_SHOP && Randomizer_GetSettingValue(RSK_GREG_HINT) &&
    ...
    if (textId == TEXT_MARKET_GUARD_NIGHT && CVarGetInteger("gMarketSneak", 0) &&
